...
namespace TaskManagerAPI.Controllers
{
    ...
    public class TasksController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public IActionResult Update(Guid id, [FromBody] TaskUpdateRequest request)
        {
            var updated = _service.Update(id, request.Description, request.IsCompleted);
            return updated == null ? NotFound() : Ok(updated);
        }
    ...
    public record TaskRequest(string Description);
    public record TaskUpdateRequest(string Description, bool IsCompleted);
}
